...
و PostgreSQL و `open-raid-z` (مُنقّح 2026-07-11): نقل رباعي التكرار
(TCP-IP/UDP-IP/QUIC (MPQUIC)/MPTCP أو SCTP) وكتابة قاعدة بيانات رباعية
التكرار (PostgreSQL/aruaru-db/النسخ المتزامن متعدد المناطق/سجل تدقيق
مستقل)، مصممة لمنع فقدان بيانات العناصر المدفوعة والبيانات المالية/
الأوراق المالية في ألعاب الأونلاين ثلاثية الأبعاد. يشارك aruaru-db كطبقة
بيانات موزعة من نوع Git-on-SQL وفي النموذج الهجين لـ VersionLess API
والتحكم بالإصدارات عبر Git. حاليًا لم يُنفَّذ سوى TCP-IP/UDP-IP، والباقي
لم يبدأ بعد (التفاصيل في `README.md`/`CLAUDE.md` الخاصين بـ
`open-web-server`).

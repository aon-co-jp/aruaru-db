...
def test_is_safe_commit_id_accepts_hex_and_uuid_like_ids() raises:
...
def test_is_safe_commit_id_rejects_empty_whitespace_and_sql_injection() raises:
...
def test_is_safe_commit_id_rejects_overlong_ids() raises:
    var long_id = String("")
    for _ in range(200):
        long_id += "x"
...
def test_query_as_of_rejects_unsafe_commit_id_before_touching_python() raises:
...
def test_live_commit_and_as_of_round_trip() raises:
    ...
    var dsn_obj = os_mod.environ.get("ARUARU_DB_TEST_DSN", "")
    var dsn = String(dsn_obj)
    if dsn.byte_length() == 0:
        print("test_live_commit_and_as_of_round_trip: skipped (set ARUARU_DB_TEST_DSN to run)")
        return

    var db = AruaruDb.connect(dsn)
    ...
    var latest = db.raw().fetchval("SELECT qty FROM items WHERE id = 'sword'")
    assert_equal(String(latest), "5")

    var old = db.query_as_of_val("SELECT qty FROM items WHERE id = 'sword'", first)
